...
def create_similarity():
    '''
    Convert and encode collection of text documents to a matrix of token counts, then calculate the similarity
    '''
    data = pd.read_csv('datasets/main_data.csv')
    # creating a count matrix
    cv = CountVectorizer()
    count_matrix = cv.fit_transform(data['comb'])  # column comb
    # creating a similarity score matrix
    similarity = cosine_similarity(count_matrix)  # shape (6010, 6010)
    return data, similarity


def rcmd(m):
    '''
    get movie index, then find it's similarity with other movies, then list top 10 similar movies
    '''
    m = m.lower()
    try:
        data.head()
    ...
    if m not in data['movie_title'].unique():
        ...
        i = data.loc[data['movie_title'] == m].index[0]  # get movie index
        lst = list(enumerate(similarity[i]))  # find movie similarities
